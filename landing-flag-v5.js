 ...
 *
 * v5 极速版（解决 50s 预览超时）：
 *   - 主数据源改为 ipwho.is：免费 1000 次/天、无每分钟硬限速，一次请求同时拿到
 *     出口 IP、国家、注册国（原生/广播判定）、ISP/机房信息
 *   - 移除 ip-api（45 次/分钟硬限速）与 RIPE 查询，彻底绕开限速瓶颈
 *   - 同一服务器只探测一次（host 去重），请求数 ≈ 服务器数
 *   - 首次探测几十台服务器约 10~20 秒，稳定在 50 秒预览超时线内；之后全部命中缓存秒开
 *   - 类型判定基于 hosting/datacenter/云厂商关键词；原生/广播 = 物理国家 vs 注册国
 */

const CACHE_KEY = "landing_flag_sub_tag_v6";
...
// 类型仅两分：数据中心 / 家宽（基于 hosting/机房/云厂商关键词推断）
function ipType(d) {
  if (d?.hosting === true) return "数据中心";
  const s = String((d?.as || "") + " " + (d?.isp || "") + " " + (d?.org || "")).toLowerCase();
  if (/(tencent|alibaba|aliyun|amazon|aws|google|microsoft|azure|digitalocean|oracle|huawei|vultr|linode|akamai|ovh|cloudflare|server|host|colo|leaseweb|contabo|ionos|m247|choopa|psychz|racknerd|hostinger|buyvm|frantech|hetzner|dmit|datapacket|datacenter|data center|cloud)/.test(s)) return "数据中心";
  return "家宽";
}
...
async function operator(proxies = []) {
  ...
  const rpm = Math.max(0, parseInt($arguments.rpm ?? "0", 10));
    ...
    const results = await mapLimit(missInternal, concurrency, async (node, k) => {
      ...
      const task = (async () => {
        try {
          ...
          if (!w?.success) throw new Error("ipwho.is failed");

          const ip = w.ip || "";
          const geoCC = String(w.country_code || "").trim().toUpperCase();
          const rirCC = String(w.connection?.country_code || "").trim().toUpperCase();
          const as = w.connection?.asn ? `AS${w.connection.asn}` : "";
          const isp = w.connection?.isp || w.connection?.org || "";
          const org = w.connection?.org || w.connection?.isp || "";
          const hosting = /(hosting|datacenter)/i.test(JSON.stringify(w)) ? true : false;

          const m = {
            flag: flagEmoji(geoCC),
            cc: geoCC,
            type: ipType({ as, isp, org, hosting }),
            native: nativeLabel(geoCC, rirCC),
          };
